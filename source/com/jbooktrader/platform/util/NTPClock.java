...
public class NTPClock {
    ...
    private class NTPClockPoller implements Runnable {
        public void run() {
            try {
                ...
                eventReport.report(AtomicTrader.APP_NAME, ERROR_MSG + host.getHostName() + ": " + e.getMessage());
            }
        }
    }
    ...
    public NTPClock() throws JBookTraderException {
        ...
        eventReport.report(AtomicTrader.APP_NAME, msg.toString());
        eventReport.report(AtomicTrader.APP_NAME, "NTP clock initialized. Offset: " + offsetNow + " ms");
